            ...
            <!-- preenchendo os dias automaticamente  $prazo vem do datab-->
              ...
              <script>
                ...
                $('#dataEntregaInp').change(function(){ 
                    let dataPedido = new Date($('#dataPedidoInp').val());
                    let dataEntrega = new Date($(this).val());

                    // impede continuar se a entrega for antes da data do pedido
                    if (dataEntrega < dataPedido) {
                    alert('A data de entrega deve ser igual ou posterior à data do pedido!');
                    $(this).val(''); // limpa o campo
                    return;
                    }

                    // mantém lógica de datas bloqueadas
                ...
                // também impede pelo lado do navegador
                $('#dataPedidoInp').on('change', function() {
                    let dataPedido = $(this).val();
                    $('#dataEntregaInp').attr('min', dataPedido); // define mínimo permitido
                });
              </script>
            ...
            <div class="janelaAplicacoes janelaAppstyle1" style='display:none'>
                <div>
                    <div class="row" style="padding:0px;margin:0px">
                        ...
                        <div class="col-sm-12" id='janelaPedidoExpresso'></div>
                        <!-- OBSERVAÇÕES -->
                                    ...
                                    <script>
                                        $.ajax({
                                            ...
                                            success: function(data) 
                                            {
                                                ...
                                                {
                                                    if (value.categoriaPai.id === 8743969) 
                                                    {
               ...
               //PREENCHENDO O JSON (só se caso tenha algum id de cliente já escolhido puxa os dados e mostra, só pra isso esse script)
                $.ajax({
                                        ...
                                        success: function(data)
                                        {
                                            ...
                                            var enderecoCliente =  dadosCliente[0].string.replace("'", "");
...
<!-- ---------------------------------------------------------------------------------------------------------- -->
<script>
// Função para carregar e definir o valor salvo no <input type="checkbox"> para um item específico
// Função para carregar o estado do checkbox de Pedido Expresso ao carregar a página
function carregarPedidoExpresso(idMoldeAtual) {
    const pedidoExpressoSalvo = localStorage.getItem('pedidoExpresso_' + idMoldeAtual) === 'PEDIDO EXPRESSO';
    $('#expresso_' + idMoldeAtual).prop('checked', pedidoExpressoSalvo);
    pedido.produtos['item_' + idMoldeAtual].pedidoExpresso = pedidoExpressoSalvo ? "PEDIDO EXPRESSO" : "";
}

// Função para atualizar o valor da caixa de seleção no pedido e salvar no localStorage
function atualizarPedidoExpresso(idMoldeAtual) {
    const isExpresso = $('#expresso_' + idMoldeAtual).is(':checked');
    pedido.produtos['item_' + idMoldeAtual].pedidoExpresso = isExpresso ? "PEDIDO EXPRESSO" : "";
    localStorage.setItem('pedidoExpresso_' + idMoldeAtual, isExpresso ? "PEDIDO EXPRESSO" : "");
}

// Chama carregarPedidoExpresso para cada ID de molde quando a página é carregada
$(document).ready(function() {
    const idsMoldes = [/* insira a lista de ids de moldes aqui */];
    idsMoldes.forEach(idMoldeAtual => {
        carregarPedidoExpresso(idMoldeAtual);
    });
});
...
$(wrapper2).on("click", ".appMoldeBtn", function(e){ 
    ...
    $.each(pedido.produtos['item_'+idMoldeAtual].aplicacoes, function(key,value){
     ...
     const checkboxExpressoHtml = `
        <label style="display: flex; align-items: center; font-size: 12px; color: white; background-color:#3a3b3c; padding: 5px; border-radius: 4px;">
            <input type="checkbox" name="expresso_${idMoldeAtual}" id="expresso_${idMoldeAtual}" 
                onchange="atualizarPedidoExpresso(${idMoldeAtual})" style="margin-right: 8px;">
            Pedido Expresso
        </label>`;

    // Limpa a div #janelaPedidoExpresso e insere a nova caixa de seleção
    $("#janelaPedidoExpresso").empty();
    $("#janelaPedidoExpresso").append(checkboxExpressoHtml);

    // Carrega o valor salvo no localStorage para a caixa de seleção criada
    carregarPedidoExpresso(idMoldeAtual);
    }); 
