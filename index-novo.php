...
// Receber as datas enviadas pelo formulário (ou usar os padrões)
$dataInicio = $_POST['dataInicio'] ?? date('Y-m-01');
$dataFim = $_POST['dataFim'] ?? date('Y-m-t');

// Validação e formatação (para segurança)
$dataInicio = date('Y-m-d', strtotime($dataInicio));
$dataFim = date('Y-m-d', strtotime($dataFim));

// Use as datas no seu código de consulta (como no exemplo anterior)
$query_base = "SELECT * FROM `Esteira` WHERE `historicoPedido` IS NOT NULL";
$resultado_base = mysqli_query($conn, $query_base);
$data = [];

// Processamento como mostrado antes
while ($row = mysqli_fetch_assoc($resultado_base)) {
    $historico = json_decode($row['historicoPedido'], true);
    if (isset($historico['historicoSetor'])) {
        foreach ($historico['historicoSetor'] as $evento) {
            $dataEvento = DateTime::createFromFormat('d/m/Y', $evento['data']);
            if ($dataEvento) {
                $dataEventoFormatada = $dataEvento->format('Y-m-d');
                if ($dataEventoFormatada >= $dataInicio && $dataEventoFormatada <= $dataFim) {
                    $data[] = $row;
                    break;
                }
            }
...
<div class="filtro-periodo">
    <form method="POST" action="">
        <label for="dataInicio">Início do período</label>
        <input type="date" id="dataInicio" name="dataInicio" value="<?php echo ($dataInicio); ?>">
        
        <label for="dataFim">Fim do período</label>
...
<script>
  // Converte o array PHP para JSON e imprime no console
